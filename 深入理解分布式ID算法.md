...
> 雪花Id重要的是上述分析中的设计理论，至于机器Id的分配并没有统一方案，这是一个需要考虑的问题，但也是需要根据各自场景去实现不同的方案设计，只要能够确保分布式，同一时间不同worker之间不重复即可。正因如此，加之一些可按需调整的内容，有了下列为例的一些雪花算法不同的实现方案，我们也可以按需实现自己的方案。
