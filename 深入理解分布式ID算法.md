# 深入理解分布式唯一ID
> 关键词: 分布式 唯一
> 目标: 可以自主分析、选用乃至设计的分布式唯一id方案

## 背景和引入
在分布式技术理论学习和技术架构设计时，分布式唯一Id成为理论之后，实践的第一个课题。在功能性为目标的业务实现初期，对承载大量并发、系统高可用性等要求暂未体现，但当经过深思熟虑决定引入分布式架构时，就绕不过这两个话题。Id的设计对于一个系统的开发阶段中是一项非常靠前的任务，根据业务自主分析、选用合理的分布式唯一Id方案将为后续达成功能和性能要求打下基础。
