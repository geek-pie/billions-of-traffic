...
> **重点**
>  
> 可以说实际上snowflake的本质原理依然可以归纳到步长+号段。最高位为毫秒时间戳，对于每一个worker而言，即自动维护了1ms的步长; 而高位的时间戳+workerId，就提前协定分配了每毫秒中可用的号段
> 
> 实际上，我们的根本需求是维护一个高度可靠的自增序列，以此来维护本机内时间维度上的号段。雪花算法的精妙之处，是在于借助了计算机系统成熟通用的时间戳来维护上述需求，无需额外定义概念和实现。
> 
>  ***
...
>   - 调用链路长度: 假设如Q2中的方案直接不使用sequenceId，异常恢复时间也缩短至最小。但加长时间戳粒度更小意味着获取时间戳的频率变高，相对于获取一次时间戳和sequenceId增加一次，前者的方法调用更长，整体的id生成速度变慢。
>   
