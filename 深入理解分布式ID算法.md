...
##### ***待解决问题*** 机器Id(workId)的分配
> 雪花Id重要的是上述分析中的设计理论，至于机器Id的分配并没有统一方案，这是一个需要考虑的问题，但也是需要根据各自场景去各自实现的内容，可以有不同的方案设计。因此这并没有统一的方案，只要能够确保分布式，同一时间不同worker之间不重复即可。也正是这样，加之一些可按需调整的内容，才有了下列为例的一些雪花算法不同的实现方案，或者按需实现自己的方案。
